...
class MockInputEvent : public input_event::InputEvent_I
{
  public:
    MOCK_METHOD(void, registerCallback,
                (std::function<void()>, input_event::input_key), (override));
...
class MockGraphics : public Graphics_I
{
  public:
    ...
    MOCK_METHOD(std::shared_ptr<Sprite_I>, createSprite, (float), (override));
    MOCK_METHOD(std::shared_ptr<Text_I>, createText, (const char *, uint8_t),
                (override));
    MOCK_METHOD(std::shared_ptr<uint32_t>, getWindowWidth, (),
                (const, override));
    MOCK_METHOD(std::shared_ptr<uint32_t>, getWindowHeight, (),
                (const, override));
    MOCK_METHOD(void, setResizeEventCallback,
                (std::function<void(uint32_t, uint32_t)>), (override));
};
